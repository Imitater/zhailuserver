...
public class HelpTakeInfoPresenter extends  HelpTakeInfoContract.Presenter{

    @Override
    public void orderInfo(String order_id, String cate_id, MultipleStatusView multipleStatusView) {
        rxManager.addObserver(RetrofitManager.getInstance().doRequest(mModel.orderInfo(order_id,cate_id), new RxObserverListener<TakeOrderInfoBean>(mView) {
            @Override
            public void onSuccess(TakeOrderInfoBean result) {
                mView.orderInfo(result);
            }
        }));
    }
