...
public class ConfirmedAdapter extends BaseQuickAdapter <PutOrderBean.OrdersListBean, BaseViewHolder>{
    public ConfirmedAdapter(int layoutResId, @Nullable List<PutOrderBean.OrdersListBean> data) {
    ...
    protected void convert(BaseViewHolder helper, final PutOrderBean.OrdersListBean item) {
        if (item.getCate_id().equals("11")) {
            ...
            helper.setVisible(R.id.adapter_item1,true);
            //物品
            helper.setText(R.id.adapter_order_sex, "所需服务：" + item.getDemand());
            //揽件时间
            helper.setText(R.id.adapter_allorder_gettime, "揽件时间：" + item.getDelivery_time());
            //取件地址
            helper.setText(R.id.adapter_order_address, "取件地址：" + item.getEnd_address() + item.getEnd_detail());
        ...
        if (item.getProgress().equals("4")||item.getProgress().equals("5")){
            helper.setVisible(R.id.adapter_order_finish_tv,true);
            LinearLayout adapterFinish = helper.getView(R.id.adapter_finish);
            adapterFinish.setVisibility(View.GONE);
        }else{
            helper.setVisible(R.id.adapter_order_finish_tv,false);
            LinearLayout adapterFinish = helper.getView(R.id.adapter_finish);
            adapterFinish.setVisibility(View.VISIBLE);
            helper.setText(R.id.adapter_order_bt,"确认完成");
        }
        helper.addOnClickListener(R.id.adapter_order_bt);
    }
